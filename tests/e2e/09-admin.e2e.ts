...
describe('09 · Admin', () => {
  ...
  it('PUT /api/admin/projects/:id/approve → 200 project approved', async () => {
    ...
    // approve uses PUT; our post() helper uses POST — use fetch-based call
    const res = await fetch(
      `${process.env['E2E_BASE_URL'] ?? 'http://localhost:3011'}/api/admin/projects/${projectId}/approve`,
      {
        method: 'PUT',
        headers: {
    ...
    void status; // silence unused warning from the post() call above
    void body;
  });
  ...
  it('PUT /api/admin/users/:id/ban → 200 user banned', async () => {
    const res = await fetch(
      `${process.env['E2E_BASE_URL'] ?? 'http://localhost:3011'}/api/admin/users/${targetUserId}/ban`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  it('PUT /api/admin/users/:id/unban → 200 user unbanned', async () => {
    const res = await fetch(
      `${process.env['E2E_BASE_URL'] ?? 'http://localhost:3011'}/api/admin/users/${targetUserId}/unban`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
